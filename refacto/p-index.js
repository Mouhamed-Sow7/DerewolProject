...
import { getPrinterBySlug, createFileGroup, uploadFileToGroup, updateFilesCount, fetchGroupsByOwner } from '../../lib/supabase';
...
// ── Preview PDF via signed URL ────────────────────────────────
async function openFilePreview(storagePath) {
...
function GroupCard({ group }) {
  ...
  // Nombre de fichiers : réel si dispo, sinon files_count sauvegardé
    ...
    <div className={`db-card ${status === 'expired' ? 'db-card--expired' : ''}`}>
      <div className="db-card-header">
        <div className="db-card-header-left">
          ...
          <div>
            <div className="db-card-count">{fileCount} fichier{fileCount > 1 ? 's' : ''}</div>
      ...
      <div className="db-files-list">
        {files.map(f => (
          <div key={f.id} className="db-file-row">
            ...
            {f.storage_path && (
              <button
                onClick={() => openFilePreview(f.storage_path)}
                style={{
                  background: 'none', border: 'none', cursor: 'pointer',
                  padding: '2px 6px', borderRadius: 6,
                  color: '#1d4ed8', fontSize: 16, lineHeight: 1,
                }}
        ...
        {/* Fichiers supprimés mais count connu */}
        {files.length === 0 && fileCount > 0 && (
          <div style={{ fontSize: 12, color: '#9ca3af', fontStyle: 'italic', padding: '4px 0' }}>
            {fileCount} fichier{fileCount > 1 ? 's' : ''} — supprimé{fileCount > 1 ? 's' : ''} après impression
          </div>
        )}
      </div>
...
export default function PrinterSPA({ showToast }) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      await updateFilesCount(groupId, selected.length); // ← persist count
        ...
        <div className="upload-card">
          <div className="header" style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{
                background: '#f5c842', color: '#111510',
                borderRadius: 8, padding: '4px 12px',
