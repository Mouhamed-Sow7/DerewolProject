...
contextBridge.exposeInMainWorld("viewer", {
  ...
  onData: (cb) =>
    ipcRenderer.once("viewer:data", (_, d) => cb(d)),

  // Main → viewer: TTL expired signal
  onTTLExpired: (cb) =>
    ipcRenderer.on("viewer:ttl-expired", () => cb()),
  ...
  print: (jobId, fileId) =>
    ipcRenderer.invoke("viewer:print", jobId, fileId),

  // viewer → Main: close and delete tmp (fire-and-forget)
  close: (jobId, fileId) =>
    ipcRenderer.send("viewer:close", jobId, fileId),
});
