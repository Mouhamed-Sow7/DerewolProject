...
function encodeFileBase64(filePath) {
  ...
  if (buffer.length > MAX_IMAGE_B64_BYTES) {
    console.warn(
      `${LOG_PREFIX} Fichier trop grand (${buffer.length} bytes), analyse texte uniquement`,
    );
    return null;
  }
...
Réponds UNIQUEMENT en JSON valide, sans markdown, sans preamble, sans backticks.`;
  ...
  const rules = {
    image:
      "Analyse la qualité visuelle, détecte si NB suffit, vérifie l'orientation optimale.",
    pdf: "Vérifie la mise en page, colonnes, marges, densité de texte, graphiques couleur.",
    excel:
      "Identifie les problèmes de formatage, colonnes mal alignées, données tronquées. Si des corrections sont nécessaires, génère une macro VBA dans le champ macroVBA.",
...
async function analyzeDocumentForPrint(filePath, anthropicApiKey) {
  ...
  console.log(
    `${LOG_PREFIX} ✅ Analyse terminée → colorMode=${analysis.colorMode} orientation=${analysis.orientation}`,
  );
