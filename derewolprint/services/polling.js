...
async function expireStaleGroups(printerId) {
  try {
    ...
    let updateQuery = supabase.from('file_groups').update({ status: 'expired' }).in('id', groupIds).eq('status', 'waiting');
    if (printerId) updateQuery = updateQuery.eq('printer_id', printerId);
    await updateQuery;

    await supabase.from('print_jobs').update({ status: 'expired' }).in('group_id', groupIds).eq('status', 'queued');

    for (const groupId of groupIds) {
      const { data: files } = await supabase.from('files').select('storage_path').eq('group_id', groupId);
      if (files?.length) {
        const paths = files.map(f => f.storage_path).filter(Boolean);
        if (paths.length) await supabase.storage.from('derewol-files').remove(paths);
      }
    }
...
async function fetchPendingJobs(printerId) {
  ...
  if (error) { console.log('[POLLING] Erreur :', error.message); return []; }

  // Filtre côté JS pour être sûr (Supabase nested filter pas toujours fiable)
  const filtered = (data || []).filter(job => 
    !printerId || job.file_groups?.printer_id === printerId
  );

  if (filtered.length > 0) console.log('[POLLING] Jobs actifs :', filtered.length);
  return filtered;
}

function startPolling(onJobsReceived, printerId, intervalMs = 10000) {
  console.log(`[POLLING] Démarrage — printer: ${printerId} — intervalle ${intervalMs/1000}s`);
