...
function renderHistory() {
  ...
  let filtered = historyData.filter(h => {
    if (historyFilters.client && !h.display_id?.toLowerCase().includes(historyFilters.client.toLowerCase())) return false;
    if (historyFilters.status && h.status !== historyFilters.status) return false;
...
document.getElementById('filter-client').addEventListener('input', e => {
  historyFilters.client = e.target.value; renderHistory();
});
document.getElementById('filter-date').addEventListener('change', e => {
  historyFilters.date = e.target.value; renderHistory();
});
document.getElementById('filter-status').addEventListener('change', e => {
  historyFilters.status = e.target.value; renderHistory();
});
...
function initSettings() {
  ...
  window.derewol.getPrinters().then(printers => {
    ...
    sel.innerHTML   = '<option value="">Auto-détection</option>' +
      real.map(p => `<option value="${p.name}" ${settings.printer === p.name ? 'selected' : ''}>${p.name}</option>`).join('');
  });
}

document.getElementById('setting-darkmode').addEventListener('change', e => {
  settings.darkmode = e.target.checked; applyDarkMode(settings.darkmode); saveSettings();
});
...
document.getElementById('setting-polling').addEventListener('change', e => {
  settings.polling = parseInt(e.target.value); saveSettings();
});
...
function showModal(title, subtitle) {
  ...
  document.querySelector('#modal-overlay .modal p').textContent  = subtitle || '';
...
async function confirmReject() {
  ...
  if (pendingReject.type === 'group') {
    ...
    if (group) {
      group.items = group.items.filter(i => !(i.jobId === jobId && i.fileId === fileId));
      if (group.items.length === 0) { animateRemove(groupId); jobStore.removeJob(groupId); }
      else {
        const t = document.querySelector(`#${groupId} .job-time`);
        if (t) t.textContent = `${group.time} · ${group.items.length} fichier${group.items.length > 1 ? 's' : ''}`;
...
function confirmJob(groupId, group) {
  ...
  if (btnReject) { btnReject.disabled = true; btnReject.style.opacity = '0.3'; }
  card.querySelectorAll('.btn-reject-file').forEach(b => { b.disabled = true; b.style.opacity = '0.3'; });
  ...
  btnCancel.addEventListener('click', () => {
    ...
    btn.textContent = '🖨️ Imprimer tout'; btn.disabled = false; btn.style.opacity = '1';
    if (btnReject) { btnReject.disabled = false; btnReject.style.opacity = '1'; }
    card.querySelectorAll('.btn-reject-file').forEach(b => { b.disabled = false; b.style.opacity = '1'; });
    btnCancel.remove();
  });
...
async function initQRView() {
  ...
  if (!qrConfig) {
    document.getElementById('qr-shop-name').textContent = 'Configuration manquante';
  ...
  if (!qrInitialized) {
    ...
    document.getElementById('btn-download-qr').addEventListener('click', () => downloadQR(qrConfig));
...
function downloadQR(cfg) {
  ...
  // ── Logo haut ─────────────────────────────────────────────
  const sqSize = 64, sqX = (W / 2) - 120, sqY = 80;
  ctx.fillStyle = '#f5c842';
  roundRect(ctx, sqX, sqY, sqSize, sqSize, 14); ctx.fill();
  ...
  ctx.fillText('Print', sqX + sqSize + 16 + ctx.measureText('Derewol').width, textY);

  // ── QR centre ─────────────────────────────────────────────
  const sourceCanvas = document.getElementById('qr-canvas');
  const qrSize = 500, qrX = (W - qrSize) / 2, qrY = 220;

  ctx.fillStyle = '#ffffff';
  roundRect(ctx, qrX - 24, qrY - 24, qrSize + 48, qrSize + 48, 20); ctx.fill();
  ctx.strokeStyle = '#d4dbd2'; ctx.lineWidth = 2;
  roundRect(ctx, qrX - 24, qrY - 24, qrSize + 48, qrSize + 48, 20); ctx.stroke();
...
window.derewol.getPrinters().then(printers => {
  ...
  const preferred  = settings.printer || real.find(p => p.name.toLowerCase().includes('hp'))?.name;
...
jobStore.subscribe(groups => {
  ...
  renderJobs(filtered, { onPrint: confirmJob, onReject: handleRejectGroup, onRejectFile: handleRejectFile });
});
...
document.getElementById('btn-reject-selected').addEventListener('click', () => {
  ...
  pendingReject = { type: 'group-multi', groupIds: [...checked].map(cb => cb.dataset.id) };
  showModal(
