...
function buildOCRSystemPrompt(language = "fr") {
  const langLabel =
    { fr: "français", en: "English", wo: "wolof" }[language] ?? "français";
...
async function extractTextFromImage({
  ...
  console.log(
    `${LOG_PREFIX} extractTextFromImage("${filePath}", lang=${language})`,
  );
  ...
  console.log(
    `${LOG_PREFIX} Réponse OCR brute (${rawText.length} chars) :`,
    rawText.slice(0, 200),
  );
  ...
  if (onCreditDeduct && userId) {
    try {
      ...
      console.log(
        `${LOG_PREFIX} ${OCR_CREDIT_COST} crédit(s) déduit(s) pour userId=${userId}`,
      );
    } catch (creditErr) {
      console.error(
        `${LOG_PREFIX} Déduction crédits échouée :`,
        creditErr.message,
      );
      ocrResult.warnings = ocrResult.warnings ?? [];
      ocrResult.warnings.push(
        "Déduction de crédit échouée — vérifiez votre solde",
      );
    }
  }

  console.log(
    `${LOG_PREFIX} ✅ OCR terminé → contentType=${ocrResult.contentType} confidence=${ocrResult.confidence}`,
  );
