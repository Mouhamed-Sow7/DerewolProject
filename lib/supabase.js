...
export async function fetchGroupsByOwner(ownerId) {
  const { data, error } = await supabase
    ...
    .select(
      ...
      print_jobs ( id, status, copies_requested, copies_remaining, expires_at, created_at )
    `,
    )
    .eq("owner_id", ownerId)
    .neq("status", "deleted");
  ...
  // PRIORITÉ: utiliser print_jobs.status au lieu de file_groups.status
  const enhanced = data.map((group) => ({
    ...group,
    status: group.print_jobs?.[0]?.status || group.status,
  }));
